...
public class SumFun extends AbstractInvariant implements IFunction {
    ...
    // Có vấn đề : Sao chưa sử dụng tới biến val???
    // Hình như bởi vì : mỗi function trong đó đều đã được tính (tức lan truyền rồi)
    @Override
    public void propagateInt(VarIntLS x, int val) {
        ...
        int nv = 0;
        // Ở đây là lấy giá trị cũ trước đó, do các function đã được gán rồi. Sau đó
        // SumFun mới được gán
        int t = x.getOldValue();
        Vector<IFunction> F = _map.get(x);
        for (IFunction f : F) {
            nv -= f.getAssignDelta(x, t); // Hay
