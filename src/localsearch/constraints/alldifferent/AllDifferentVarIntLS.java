...
public class AllDifferentVarIntLS extends AbstractInvariant implements IConstraint {
    ...
    public AllDifferentVarIntLS(VarIntLS[] x) {
        ...
        // ??? Có vẻ chưa đúng, bởi vì : _minValue, _maxValue ban đầu chưa được gán
        // tương ứng các giá trị MAX_INT, hay MIN_INT ==> Vì thế việc so sánh này sẽ
        // dẫn đến sai sót ????
        for (int i = 0; i < x.length; i++) {
            _minValue = Math.min(_minValue, x[i].getMinValue());
            _maxValue = Math.max(_maxValue, x[i].getMaxValue());
        }
        // ??? Biến _posted này thực sự có ý nghĩa gì ???
        _posted = false;
        post();
    ...
    private void post() {
        ...
        // Tạo ra một mảng với số phần tử bằng số giá trị lớn nhất trong miền từ 
        // _minValue tới _maxValue. Mảng này dùng để đếm số lần xuất hiện của 1 giá trị
        // Từ đó có thể biết được có giá trị này xuất hiện nhiều hơn 1 lần (tức vi phạm)
        _occ = new int[_maxValue - _minValue + 1];
        for (int i = 0; i < _occ.length; i++) {
            _occ[i] = 0;
        }
        
        // Tạo ra một hash map từ VarIntLS sang Integer (số integer này cho biết chỉ số
        // của phần tử đó trong mảng VarIntLS[])
        _mapVariableIndex = new HashMap<VarIntLS, Integer>();
        for (int i = 0; i < _x.length; i++) {
            _mapVariableIndex.put(_x[i], i);
        }
        
        // Gọi phương thức post của LocalSearchManager để lưu lại constraint này. Nhiệm
        // vụ chính của ls chính là : lưu toàn bộ các đối tượng có kiểu là 
        // AbstractInvariant (là một lớp cha của các lớp dạng Ràng buộc và Function)  
        _ls.post(this);
        _posted = true;
    }
    ...
    // Số vi phạm của một biến (chính bằng số lần trùng lặp trừ 1)
    ...
    public void propagateInt(VarIntLS x, int val) {
        ...
        // Vì chỉ có 1 biến bị thay đổi nên violation cùng lắm chỉ giảm được 1 đơn vị
        if (_occ[v1] > 1) {
            _violations--;
        }
        _occ[v1]--;
    ...
    public void print() {
        ...
        // ? Nếu _minValue âm thì sao? chỉ số âm thì ko được, đoạn code 
        // này viết nhầm : phải chạy từ 0 tới maxValue-minValue
        for (int v = _minValue; v <= _maxValue; v++) {
            System.out.println("_occ[" + v + "] = " + _occ[v]);
        }
    }

    // Tại sao lại phải cần đến hàm này? Tại sao phải mất công kiểm tra như vậy?
    @Override
    public boolean verify() {
        ...
        //??? Đoạn này hình như viết nhầm : phải từ 0 tới maxValue-minValue chứ?
        for (int v = _minValue; v <= _maxValue; v++) {
            if (_occ[v] != occ[v]) {
