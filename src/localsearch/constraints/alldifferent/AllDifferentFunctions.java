...
public class AllDifferentFunctions extends AbstractInvariant implements IConstraint {
    ...
    public int violations(VarInt x) {
        ...
        int violations = 0;
        for (Integer e : h) {
            violations += Math.max(0, _occ[e] - 1);
        }
        return violations;
    }
    ...
    public int getAssignDelta(VarIntLS x, int val) {
        ...
        for (IFunction f : F) {
            ...
            // ??? Đây chỉ là gán thử sao lại thay đổi trực tiếp như thế này ???
            _occ[v]--;
        }
        
        // Chưa hiểu lắm ý tưởng
        for (IFunction f : F) {
            int v = f.getValue() + f.getAssignDelta(x, val) - _minValue;
            if (_occ[v] > 0) {
                nv++;
            }
            // ??? Tại sao lại được phép thay đổi trực tiếp ntn?
            _occ[v]++;
        }
