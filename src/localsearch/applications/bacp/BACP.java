...
public class BACP {
    ...
    public static void main(String[] args) {
        ...
        int n = 46; // number of courses (tổng số môn học)
        int m = 8; // number of periods (số kỳ học)
        ...
        //(b[i],e[i]) is an edge of prerequisite: course b[i] must be a prerequisite of course e[i]
        int[] b;
        int[] e;
        //m.pp();
        //n=46;
        ...
        // Thể hiện cặp môn học có quan hệ tiên quyết, b[i] phải được học trước e[i]
        ...
        // ???? Tìm hiểu thêm về tổng có điều kiện
        ...
        // ??? Ở đây có có xây dựng thêm 1 ràng buộc về minimize (tức cân bằng về khối lượng
        // học phần giữa các kỳ học)
        Max max = new Max(s_credit);
        Min min = new Min(s_credit);
        FuncMinus mm = new FuncMinus(max, min);

        // ??? Bình thường chỉ có ls.close, làm gì có S.close()???
        S.close();
        mgr.close();
         ...
         */
        // Chú ý tới hàm này : Khi có thêm ràng buộc mm (tức cân bằng khối lượng học tập
        // giữa các kỳ học)
